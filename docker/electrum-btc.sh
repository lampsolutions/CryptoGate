...
PUBKEY=$BTC_PUBKEY
MODE=$BTC_MODE
ELECTRUM_FORK=electrum-btc
ELECTRUM_BINARY="/opt/electrum/run_electrum"
...
if [ $MODE == "testnet" ]; then
    ...
    ELECTRUM_OPTS="--dir $ELECTRUM_DATA_DIR_CMD --testnet -v99 -w $ELECTRUM_WALLET"
else
    ELECTRUM_DATA_DIR="/data/$ELECTRUM_FORK"
    ELECTRUM_WALLET_DIR="$ELECTRUM_DATA_DIR/wallets"
    ELECTRUM_WALLET="$ELECTRUM_WALLET_DIR/default_wallet"
    ELECTRUM_OPTS="--dir $ELECTRUM_DATA_DIR_CMD -v99 -w $ELECTRUM_WALLET"
fi
...
if [ ! -f $ELECTRUM_WALLET ]; then
    $ELECTRUM_BINARY restore $PUBKEY $ELECTRUM_OPTS
fi
